...
test("冒烟: 插件工具链（create + list + remove）", async () => {
  ...
  patchWikiConfig(process.platform === "darwin" ? join(root, "Library", "Application Support", "pi")
  : process.platform === "linux" ? join(process.env.XDG_CONFIG_HOME ?? join(root, ".config"), "pi")
  : join(root, "AppData", "Roaming", "pi"), { selfModifyPlugins: true });
